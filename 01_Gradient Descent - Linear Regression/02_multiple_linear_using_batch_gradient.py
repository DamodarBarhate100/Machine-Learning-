...
def batch_gradient_decent(x,y):
    ...
    for i in range(iterations):
        y_predicted = x_with_bias @ theta_j
        errors = y_predicted - y
        cost_funct = (1/(2*m)) * np.sum(np.square(errors))
        gradient = (1/m) * (errors @ x_with_bias)
        theta_j = theta_j - learning_rate * gradient

        if(i%20 == 0):
            print("m:{} b:{} cost:{} i:{}".format(theta_j[1:],theta_j[0],cost_funct,i))
    return theta_j[1:] ,theta_j[0]

m_scaled,b_scaled = batch_gradient_decent(x_scaled,y_scaled)
